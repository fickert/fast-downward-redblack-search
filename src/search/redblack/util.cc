...
void add_num_black_options(options::OptionParser &parser) {
	parser.add_option<int>("num_black", "number of variables to be painted black", "1", options::Bounds("0", "infinity"));
...
auto get_num_black(const options::Options &opts) -> int {
	...
	if (!opts.contains("num_black") || opts.get<int>("num_black") < 1) {
		assert(opts.contains("ratio_black"));
		return std::min(num_variables, std::max<int>(1, num_variables * opts.get<double>("ratio_black")));
	}
