...
RedBlackDAGFactFollowingHeuristic::RedBlackDAGFactFollowingHeuristic(const Options &opts)
	...
	if (!extract_plan && applicable_paths_first){
		std::cout << "Cannot set applicable_paths_first when not extracting the plan -- need to maintain the current state" << endl;
...
void RedBlackDAGFactFollowingHeuristic::initialize() {
    std::cout << "Initializing Red-Black Relaxation heuristic..." << endl;
	...
	// from ff heuristic
	//AdditiveHeuristic<GlobalState, GlobalOperator>::initialize();
    ...
    if (num_invertible_vars == 0) {
    	std::cout << "No invertible variables - running FF heuristic!" << endl;
    	std::cout << "---------------------------------------------------------------------------------------" << endl;
...
//	}
    ...
    std::cout << "Finished initializing Red-Black Relaxation heuristic at time step [t=" << utils::g_timer << "]" << endl;
}
...
void RedBlackDAGFactFollowingHeuristic::dump_options() const {
	if (paint_roots_black) {
		std::cout << "Forcing painting root variable black" << endl;
	}

	if (ignore_invertibility) {
		std::cout << "Warning! Running with ignore_invertibility option ON is not safe - may return DEAD_END for non dead end states. " << endl;
	}
	std::cout << "Running A* instead of Dijkstra. Using the distances ignoring outside conditions for heuristic estimates." << endl;
	std::cout << "Preferred operators (if used) are set ";
	if (preferred_type == RELAXED) {
		std::cout << "from relaxed plan" << endl;
	} else if (preferred_type == EXTRA_BLACK) {
		std::cout << "from relaxed plan and applicable black changing actions" << endl;
	} else if (preferred_type == PRECS_UNCHANGED) {
		std::cout << "by unchanged preconditions along red-black plan actions" << endl;
	} else {
		std::cout << "by unknown (" << preferred_type << ")";
	}

	std::cout << "Conflict costs are estimated" << endl;
}

void RedBlackDAGFactFollowingHeuristic::mark_preferred_operators_and_relaxed_plan(const GlobalState &state, relaxation_heuristic::Proposition *goal) {
...
auto RedBlackDAGFactFollowingHeuristic::ff_compute_heuristic(const GlobalState &state) -> int {
...
void RedBlackDAGFactFollowingHeuristic::apply_while_possible() {
...
//			cout << "Applicable" << endl;
	...
	check_goal_via_state(goal_facts);
}

void RedBlackDAGFactFollowingHeuristic::create_extended_DTGs() {
    std::cout << "Initializing extended DTGs" << endl;
...
void RedBlackDAGFactFollowingHeuristic::prepare_DTGs_for_invertibility_check() {
	...
	std::cout << "Adding " << g_operators.size() << " operators to extended DTGs" << endl;
    ...
    for (int op_no = 0; op_no < g_operators.size(); op_no++) {
		...
		for (size_t i = 0; i < effects.size(); i++) {
			...
			int pre_value = get_precondition_for_variable(*op, var);
...
void RedBlackDAGFactFollowingHeuristic::get_cg_neighbours(vector<int> &neigh, int node) const {
	vector<int> pred =  causal_graph::get_causal_graph(g_root_task().get()).get_predecessors(node);
	vector<int> succ = causal_graph::get_causal_graph(g_root_task().get()).get_successors(node);
...
void RedBlackDAGFactFollowingHeuristic::get_relaxed_plan(const GlobalState &state, relaxation_heuristic::Proposition *goal) {
    if (!goal->marked) { // Only consider each subgoal once.
        goal->marked = true;
		relaxation_heuristic::UnaryOperator *unary_op = goal->reached_by;
        if (unary_op) { // We have not yet chained back to a start node.
            ...
            if (operator_no != -1) {
            	...
            	if (!relaxed_plan[operator_no]) {
					...
					// don't care about preferred operators here
...
#if defined(DEBUG_RED_BLACK) || defined(CRITICAL_RED_BLACK)
	        const GlobalOperator *op = &g_operators[op_no];
#endif
...
bool RedBlackDAGFactFollowingHeuristic::is_semi_relaxed_goal_reached(const std::vector<FactPair> &goal) {
...
int RedBlackDAGFactFollowingHeuristic::compute_heuristic(const GlobalState &state) {
	...
	if (black_indices.size() == 0)
		return ff_compute_heuristic(state);
	...
	if (next_red_action_test) {
		...
		for (int op_no=0; op_no < g_operators.size(); op_no++) {
			...
			for (size_t i = 0; i < effects.size(); i++) {
				...
				if (black_vars[var])
					continue;
				int pre_value = get_precondition_for_variable(g_operators[op_no], var);
				if (pre_value != -1) {
					for (partial_assignment::iterator it = black_eff.begin(); it != black_eff.end(); ++it) {
...
bool RedBlackDAGFactFollowingHeuristic::is_currently_applicable(const vector<int>& ops, bool skip_black) {
	if (!std::all_of(std::begin(ops), std::end(ops), [this](auto op_no) { return current_legal_operators[op_no]; }))
		return false;
...
static Heuristic<GlobalState, GlobalOperator> *_parse(OptionParser &parser) {
